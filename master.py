    ...
    while(True):
        if(len(tasks_queue) > 0):
            for task in tasks_queue:
                ...
                with open("logs/master.txt", "a+") as log_file:
                    lock.acquire()
                    log_file.write(str(datetime.now()) + "\tWorker has been sent task -> [worker_id:{0}, task_id:{1}, job_id:{1}, duration:{1}]\n".format(worker_id, task_to_schedule['task_id'], task_to_schedule['job_id'], task_to_schedule['duration']))
                    lock.release()
    ...
    with s:
        ...
        while(True):
            ...
            with conn:
                ...
                with open("logs/master.txt", "a+") as log_file:
                    lock.acquire()
                    log_file.write(str(datetime.now()) + "\tReceived update from worker -> [worker_id:{0}, task_id:{1}]\n".format(worker_id, task_id))
                    lock.release()
                ...
                if jt_count[job_id] == 0:
                    with open("logs/master.txt", "a+") as log_file:
                        lock.acquire()
                        log_file.write(str(datetime.now()) + "\tJob has finished execution -> [job_id:{0}]\n".format(job_id))
                        lock.release()
