...
class Worker:
    def __init__(self, worker_id, port):
        ...
        with open("logs/worker"+str(worker_id)+".txt", "a+") as log_file:
            log_file.write(str(datetime.now()) + ":\t\tWorker has started: (worker_id:{0}, port:{1}, slots:{2})\n".format(self.worker_id,self.port,self.slots))
    ...
    def task_to_slot(self, task_record):
        ...
        with open("logs/worker"+str(worker_id)+".txt", "a+") as log_file:
            log_file.write(str(datetime.now()) + ":\t\tTask has started EXECUTION: (job_id:{0}, task_id:{1})\n".format(task_record["job_id"],task_record["task_id"]))
        ...
        while True:
            ...
            with conn:
                ...
                if received_task_json:
                    ...
                    with open("logs/worker"+str(worker_id)+".txt", "a+") as log_file:
                        lock.acquire()
                        log_file.write(str(datetime.now()) + "\t\tThe received task: (job_id:{0}, task_id:{1}, duration:{2})\n".format(task["job_id"], task["task_id"], task["duration"]))
                        lock.release()
    ...
    while True:
        ...
        for i in range(worker.slots):
            if not worker.task_execution_pool[i]:   # implies that no task has been assigned to this slot
                ...
                with open("logs/worker"+str(worker_id)+".txt", "a+") as log_file:
                    lock.acquire()
                    log_file.write(str(datetime.now()) + ":\t\tExecution of task has FINISHED: (job_id:{0}, task_id:{1})\n".format(job_id, task_id))
                    lock.release()
                ...
                with open("logs/worker"+str(worker_id)+".txt", "a+") as log_file:
                    lock.acquire()
                    log_file.write(str(datetime.now()) + "\t\tUPDATE has been sent to master: (job_id:{0}, task_id:{1}) task has been COMPLETED\n".format(job_id, task_id))
                    lock.release()
